...
export const metadata: Metadata = {
  title: "MoodTune | Studio",
  description: "Mood search studio — describe your feeling and discover tracks that match.",
};
...
const FLOW_STEPS = [
  {
    label: "Input Layer",
    text: "Mood text, profile feedback, and future journey controls all point to the same emotional search surface.",
  },
  {
    label: "Search Engine",
    text: "Bundled embeddings shape the request, pgvector matches tracks, and a safe fallback keeps the demo responsive.",
  },
  {
    label: "Taste Memory",
    text: "Like and skip actions build a session-tied taste profile so the results start bending toward the listener over time.",
  },
  {
    label: "Journey Mode",
    text: "The old MoodTune promise stays intact: discovery should feel like path-planning through emotion, not one static recommendation.",
  },
];
    ...
    <main className="relative min-h-screen overflow-hidden bg-[var(--background)] text-[var(--foreground)]">
      ...
      <div className="relative mx-auto max-w-7xl px-4 pb-12 pt-8 sm:px-6 lg:px-8">
        ...
        <nav className="mb-6 flex flex-wrap items-center gap-3">
          <Link
            href="/"
            className="inline-flex items-center gap-2 rounded-full border border-white/12 bg-white/[0.04] px-4 py-2 text-xs uppercase tracking-[0.18em] text-neutral-300 transition hover:bg-white/[0.1]"
          >
        ...
        <section className="liquid-card mx-auto rounded-[24px] px-6 py-6 text-center sm:px-8">
          ...
          <p className="mx-auto mt-4 max-w-3xl text-sm leading-7 text-neutral-300 sm:text-base">
            ...
            studio keeps the new chat, profile, and API architecture underneath.
          </p>
          ...
          <aside className="space-y-6">
            ...
            <section className="grid gap-3">
              {SIGNALS.map((signal) => {
                ...
                return (
                  <article key={signal.title} className="liquid-card rounded-[24px] px-5 py-5">
